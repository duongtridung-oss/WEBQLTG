            ...
            <div class="row">
                ...
                <div class="col-sm-9">
                    <div class="calendar-grid">
                        <div id="weekTitle" class="fw-bold fs-5 mb-2"></div> <!-- Thêm dòng này để hiển thị tiêu đề tuần -->
...
// Biến lưu ngày hiện tại được chọn
...
function submitEventForm() {
    ...
    .then(response => {
        if (response.ok) {
            // Nếu đang chỉnh sửa, xóa sự kiện cũ
            if (editingEventId) {
...
function renderEventsOnWeekView(events) {
    ...
    events.forEach(event => {
        ...
        // Hiển thị tiêu đề in đậm, cắt ngắn nếu quá dài, có tooltip
        ...
        eventDiv.innerHTML = `<strong title="${event.title.replace(/"/g, '&quot;')}">${displayTitle}</strong>`;
            ...
            editBtn.onclick = function() {
                // Lưu id sự kiện đang chỉnh sửa
                editingEventId = data.id;
                document.getElementById('taskTitle').value = data.title;
                document.getElementById('allDayCheck').checked = data.all_day;
                document.getElementById('startDate').value = data.start_time.slice(0,16);
                if (data.end_time) {
